defmodule Perkle.Contract do
    ...
    @doc "Installs a filter on the Perkle node. This also formats the parameters, and saves relevant information to format event logs."
    ...
    defp init_events(abi) do
      ...
      names_and_signature_types_map =
        Enum.map(events, fn {name, v} ->
          ...
          encoded_event_signature = "0x#{Perkle.encode_abi_event(signature)}"
    ...
    def handle_cast({:uninstall_filter, filter_id}, state) do
      Perkle.uninstall_filter(filter_id)
          ...
          Enum.map(0..(length(topics) - 1), fn i ->
            ...
            if topic do
              if is_list(topic) do
                ...
                Enum.map(topic, fn t ->
                  "0x" <> (Perkle.encode_abi_data(topic_type, [t]) |> Base.encode16(case: :lower))
                end)
              else
                topic_type = Enum.at(topic_types, i)
                "0x" <> (Perkle.encode_abi_data(topic_type, [topic]) |> Base.encode16(case: :lower))
              end
            else
              topic
            end
      ...
      if event_data[:fromBlock] do
        new_from_block =
          if Enum.member?(["latest", "earliest", "pending"], event_data[:fromBlock]) do
            ...
            Perkle.encode_abi_data("(uint256)", [event_data[:fromBlock]])
          end
        ...
        new_param =
          if Enum.member?(["latest", "earliest", "pending"], event_data[key]) do
            event_data[key]
              ...
              # (Perkle.encode_abi_data("(uint256)", [event_data[key]]) |> Base.encode16(case: :lower))
              Perkle.to_hex(event_data[key])
          end
    ...
    defp extract_non_indexed_fields(data, names, signature) do
      Enum.zip(names, Perkle.decode_abi_event(data, signature)) |> Enum.into(%{})
    end
        ...
        if logs != [] do
          Enum.map(logs, fn log ->
            formatted_log =
              Enum.reduce(
                [
                  Perkle.abi_keys_to_decimal(log, [
                    "blockNumber",
                    "logIndex",
                    "transactionIndex",
                    # "transactionLogIndex"
    ...
    #   {:ok, receipt} = Perkle.get_transaction_receipt_by_hash(tx_hash)
    ...
    #           Enum.zip(
    #             event_attributes[:non_indexed_names],
    #             Perkle.decode_abi_event(log["data"], event_attributes[:signature])
    #           )
    #           |> Enum.into(%{})
    #         if length(log["topics"]) > 1 do
    #           [_head | tail] = log["topics"]
